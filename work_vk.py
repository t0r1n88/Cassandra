...
def calculate_data():
    ...
    # Создаем список в котором будем хранить все словари с посчитанным результатом для каждого файла
    itog_lst = []
    ...
    for file in names_files_data:
        try:
            ...
            temp = openpyxl.load_workbook(f'{file}', read_only=True)
           ...
           # Создаем словарь верхнего уровня
            ...
            for sheet in sheets:
                ...
                df = pd.read_excel(f'{file}', sheet_name=sheet, skiprows=8,
            ...
            # Добавляем в список itog_lst для последующей обработки
            itog_lst.append(itog_dct)
    ...
    try:
        ...
        # Обрабатываем список со словарями
        print(len(itog_lst))
        for data in itog_lst:
            print(len(data))
